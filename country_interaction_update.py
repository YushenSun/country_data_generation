...
for i in range(1, number_rows):
    ...
    for j, action in enumerate(action_columns):
        if action == 'A Major Political Actions':  # Example: Use the formula for the first action
            # Extract parameters from the previous row's data
            a_public_trust = current_values['A Public Trust']
            a_political_stability = current_values['A Political Stability']
            a_bilateral_diplomatic_relations = current_values['Political Similarity']
            ...
            if random_value < support_probability:
                row_actions[j] = 3  # For example, +3 indicates the action is taken (this is for A Major Political Actions)
            else:
                row_actions[j] = 0  # No action is taken

        else:
            # Default to 0 if the action isn't defined yet
            row_actions[j] = 0
    
    # Assign the row actions directly to the DataFrame
    data_actions.loc[i] = row_actions

    # For generating state data, use a simple random fluctuation model to update the state values based on the previous row
